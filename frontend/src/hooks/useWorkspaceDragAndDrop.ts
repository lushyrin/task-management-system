...
const WORKSPACE_TASKS_KEY = (workspaceId: string) => ['workspaces', workspaceId, 'tasks']
...
export function useWorkspaceDragAndDrop(tasks: Task[], workspaceId: string) {
    ...
    const { mutate: moveTask, isPending } = useMutation({
        mutationFn: ({ taskId, newStatus, newOrder }: { taskId: string; newStatus: TaskStatus; newOrder: number }) => 
            workspaceService.updateTask(workspaceId, taskId, { status: newStatus, order: newOrder }),
    ...
    const onDragEnd = useCallback(
        ...
        [activeTasks, tasks, moveTask, workspaceId]
    )
